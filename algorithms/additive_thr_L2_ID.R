additive_thr_L2_ID <-function(X, thr_const = 1,
  ...
  if (is.null(vec)){
    ...
    if (length(vec)>50){
      thr_const=l2_dimension%>%filter(dimension==50)%>%dplyr::select(threshold)%>%pull()
      thr_fin = thr_const * sqrt(2*log(nrow(X)))
    }else{
      #cat('The threshold is',thr_fin,'\n')
      thr_const=l2_dimension%>%filter(dimension==length(vec))%>%dplyr::select(threshold)%>%pull()
      thr_fin = thr_const * sqrt(2*log(nrow(X)))}
    ...
    if (e - s <= 1) {
      ...
      if (k_r < k_l) {
        while ((chp == 0) & (k_r < min(k_l, rur))) {
          ...
          #print(CUSUM_r[k_r])
          if (is.na(CUSUM_r[k_r])){cat("k_r:",k_r,"k_l",k_l,"rur:",rur,"\n")}
          #cat('the cusum is:',CUSUM_r[k_r],"\n")
      ...
      if (chp != 0) {
        if (chp > ((e + s) / 2)) {
          r <- additive_thr_L2_ID(X, s = s, e = chp, points = points,
                                  thr_fin = thr_fin, k_r = k_r,
                                  k_l = 1,check = FALSE)
        } else {
          r <- additive_thr_L2_ID(X, s = chp + 1, e = e, points = points,
                                  thr_fin = thr_fin, k_r = 1,
                                  k_l = max(1, k_l - 1),check = FALSE)
        }
        cpt <- c(chp, r)
      } else {
        cpt <- chp
