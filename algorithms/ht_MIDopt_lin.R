ht_MIDopt_lin <- function(X, thr_const = 1.4,
  ...
  cpt_inf <- ht_additive_thr_Linf_trend(X, thr_const = thr_const,
                                 thr_fin = thr_const * sqrt(2*log(nrow(X))),
  ...
  #if (SparsityEst > 0.6){
    cpt_l2 <- ht_additive_thr_L2_trend(X, thr_const = thr_const,
                                 thr_fin = thr_const * sqrt(2*log(nrow(X))),
                                 s = 1, e = nrow(X), points = points, k_l = 1,
                                 k_r = 1, scal = sc)
  return(list(cpt_linf = cpt_linf, cpt_l2 = cpt_l2))
}
